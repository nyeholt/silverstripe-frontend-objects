...
class FrontendFileCreatable extends DataExtension
{
    ...
    public function saveFileUpload($tmpFile)
    {
        ...
        $folder = DataObject::get_by_id(Folder::class, ((int)$this->creatorPage->pid));
    ...
    public function objectExists($post, $pid)
    {
        ...
        $folder = DataObject::get_by_id(Folder::class, $pid);
        $folderPath = $folder && $folder->ID ? rtrim(substr($folder->Filename, 7), '/') : '';
        ...
        if (file_exists("$base/$relativeFilePath") && $file = DataObject::get_one(File::class, "Filename = '$relativeFilePath' AND ParentID = $pid")) {
            return $file;
        }
    }
