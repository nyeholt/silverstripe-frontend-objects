...
class ObjectCreatorPage extends Page {
	...
	public function getCMSFields() {
		...
		$fields->addFieldToTab('Root.AfterSubmission', HTMLEditorField::create('SuccessMessage', 'Success Message displayed after new object creation')->setRightTitle('Use $Link, $EditLink and $Title to reference the created page'));
		$fields->addFieldToTab('Root.AfterSubmission', HTMLEditorField::create('EditingSuccessMessage', 'Success Message dislpayed after editing existing object')->setRightTitle('Use $Link, $EditLink and $Title to reference the created page'));
		...
		if ($this->CreateType) {
			...
			if (Extensible::has_extension($this->CreateType, WorkflowApplicable::class)) {
                ...
                $fields->addFieldToTab(
                    'Root.Main',
                    DropdownField::create('WorkflowDefinitionID', 'Workflow Definition', $workflows)
                        ->setHasEmptyDefault(true)->setRightTitle("Do not use if page is created in a location with workflow"),
                    'Content'
                );
